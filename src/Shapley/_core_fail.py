...
def BooleanShapleyAnalysis():
    ...
    if len(args) < 2:
        ...
        rowsofnodes, averightinrows, table, bidecimalpairs, protime = binarizeAndPropagate(formulas, formulas, inputnames, outputname, \
                    speciesnames, "abinetwork", sortedinput, sortedinter, None, mode, 15, totest = False, debug=True)
...
def BooleanShapleyTest():
    ...
    if len(args) < 2:
        ...
        if 1:
            bicount = 0
            for ainp, ainter, in oridecimalpairs.items():
                if ainter != bidecimalpairs[ainp]:
                    print(ainp, ainter, bidecimalpairs[ainp])
                    bicount += 1
            print("\n----TOTAL NUMBER OF DIFFERENCES BETWEEN ORIGINAL NETWORK AND BINARIZED NETWORK IS {}----\n".format(bicount))
        ...
        averightrows, ave_dncg = evaluation(rowsofnodes, simrowsofnodes, table, outputname)
...
def propagateCyclic(net, simtable, index, aindex, descendants, outname, formulas, simformulas, originalspecies, extranodes, masterDiamonds, inputnames, orderedBiNodes, simpercent = 0.0, debug=False):
    """
    Propagate values through the network considering diamond structures to determine the rows of nodes.
    Parameters:
        net: The directed graph
...
def binarizeAndPropagate(oriformulas, aformulas, inputnames, outputname, orispeciesnames, networkname, sortedinput, sortedinter, extranodes, mode = 'Shapley', simpercent = 15, totest=True, debug=False):
    ...
    for formula in bistrformulas:
        if debug:
            print("Binarized formula for {}: {}".format(formula['term'], formula['formula'])) 
    # bistrformulas = toBinaryFormulas(oriformulas, debug)
    binet = convertBiBooleanFormulas2Network(bistrformulas, inputnames, orispeciesnames, True, debug) 
    ...
    diamonds = diamondDigger(binet, outputname, biformulasdict, debug=True) 
    ...
    if extranodes:
        ...
        rowsofnodes = propagateCyclic(binet, table, index, aindex, \
                                    ...
                                    orderedBiNodes, simpercent=simpercent, debug=True)
        ...
        rowsofnodes = propagateAcyclic(binet, table, index, aindex, outputname, biformulasdict, masterDiamonds, inputnames, orderedBiNodes, True)
