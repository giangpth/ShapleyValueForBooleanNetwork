...
import numpy as np
import copy 
from Shapley.Node import replaceNode 


def convertBooleanFormulas2Network(formulas, inputnames, speciesnames, debug=False):
    """
    Convert boolean formulas to a directed graph using networkx.
    Parameters:
        formulas (dict): A dictionary where keys are species names and values are the root nodes of their boolean formula trees.
        inputnames (set): A set of species names that are considered input nodes (no incoming edges).
        speciesnames (set): A set of all species names in the network.
        filename (str): The name of the output HTML file for visualization.
...
def removeCycles(net, fas, possitiveOnly = True, debug = False):
    """
    Remove cycles from a directed graph by deleting edges in the feedback arc set (FAS).
    Parameters:
        net (nx.DiGraph): A directed graph. To be modified in place.
        fas (list): A list of edges in the feedback arc set (FAS) to be removed.
        possitiveOnly (bool): If True, only remove edges that are part of positive cycles. If False, remove all edges in FAS.
        debug (bool): If True, print debug information.
    """
    if debug: 
...
def convertBiBooleanFormulas2Network(biformulas, inputnames, speciesnames, filename, acyclic = False, debug=False, extranodes=None):
    """
    Convert bi-boolean formulas to a directed graph using networkx.
    Parameters:
        biformulas (list): A list of dictionaries where each dictionary has 'term' and 'formula' keys.
        inputnames (set): A set of species names that are considered input nodes (no incoming edges).
        speciesnames (set): A set of all species names in the network.
        filename (str): The name of the output HTML file for visualization.
        acyclic (bool): If True, remove cycles from the network.
        debug (bool): If True, print debug information.
...
def getFAS(net, order, debug=False):
    """
    Get an approximate minimum feedback arc set (FAS) based on a given linear arrangement of nodes.
    Parameters:
        net (nx.DiGraph): The input directed graph.
        order (list): A list of nodes representing the linear arrangement.
        debug (bool): If True, print debug information.
    Returns:
        FAS (list): A list of edges in the feedback arc set (FAS).
        FWAS (list): A list of edges not in the feedback arc set (FWAS).
    """
    FAS = []
    FWAS = []
    edges = net.edges
...
def manipulateNetwork(net, inputnames, formulas, acyclic = False, binary = False, debug=False): 
    """
    Manipulate the directed graph to remove cycles and modify the boolean formulas accordingly.
    Parameters:
        net (nx.DiGraph): The input directed graph.
        inputnames (set): A set of species names that are considered input nodes (no incoming edges).
        formulas (dict): A dictionary where keys are species names and values are the root nodes of their boolean formula trees.
        acyclic (bool): If True, remove cycles from the network.
        binary (bool): If True, treat the network as a binary network.
        debug (bool): If True, print debug information.
    Returns:
        anet (nx.DiGraph): The manipulated directed graph.
        aformulas (dict): The modified boolean formulas.
        extranodes (list): A list of extra nodes added to the network (if any).
        node_positions (dict): A dictionary mapping each node to its (x, y) position
    """
    ...
    if acyclic or binary:
        ...
        node_positions = layout_acylic(anet, aorder, False)
...
def layout_acylic(net, order, debug=False):
    """
    Arrange layout of an acyclic directed graph for visualization.
    Parameters:
        net (nx.DiGraph): The input directed acyclic graph.
        order (list): A list of nodes representing the linear arrangement.
        debug (bool): If True, print debug information.
    Returns:
        dict: A dictionary mapping each node to its (x, y) position.   
...
def layout(net, inputnames, fas, order, debug=False):
    """
    Arrange layout of the directed graph for visualization.
    Parameters:
        net (nx.DiGraph): The input directed graph, to be modified in place the positions of nodes
        inputnames (set): A set of species names that are considered input nodes (no incoming edges).
        fas (list): A list of edges in the feedback arc set (FAS).
        order (list): A list of nodes representing the linear arrangement.
        debug (bool): If True, print debug information.
