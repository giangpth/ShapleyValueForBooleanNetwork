...
def convertBiBooleanFormulas2Network(biformulas, inputnames, speciesnames, filename, acyclic = False, debug=False, extranodes=None):
    ...
    if extranodes:
        ...
        anet, aformular, extra, nodes_layer = manipulateNetwork(net, inputnames.union(set(extranodes)), biformulas, acyclic, True, debug)
    else:
        # print("Extranodes is empty")
        anet, aformular, extra, nodes_layer = manipulateNetwork(net, inputnames, biformulas, acyclic, True, debug)
    # nt.show_buttons(filter_=["physics"])
    # nt.from_nx(anet)
    
    # nt.show(filename+ ".html", notebook=False)
    # showNetwork(anet)
    return anet, nodes_layer 
...
def manipulateNetwork(net, inputnames, formulas, acyclic = False, binary = False, debug=False): 
    ...
    if acyclic or binary:
        ...
        node_layers = layout_acylic(anet, inputnames.union(set(extranodes)), fas, aorder, False)
    return anet, aformulas, extranodes, node_layers 
